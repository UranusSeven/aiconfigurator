...
OUT_GPU_OUTPUT_FRONTIER_CSV = BASE / "b300_sota_per_gpu_output_token_pareto_frontiers.csv"
OUT_GPU_OUTPUT_PNG = BASE / "b300_sota_pareto_per_user_vs_per_gpu_output_tokens.png"
...
for _, model_group in frontier_source.groupby("model_label"):
    sorted_group = model_group.sort_values(
        ["per_user_tps", "per_gpu_throughput_8gpu"], ascending=[False, False]
    )
    best_per_gpu = float("-inf")
    for idx, row in sorted_group.iterrows():
        per_gpu = float(row["per_gpu_throughput_8gpu"])
...
for model_label, model_frontier in frontier.groupby("model_label"):
    ...
    ax.plot(
        ...
        model_frontier["per_gpu_throughput_8gpu"],
        color=color,
        linewidth=2.4,
        marker="D",
        markersize=5.5,
    ...
    for _, row in model_frontier.tail(1).iterrows():
        ...
        ax.annotate(
            label,
            (row["per_user_tps"], row["per_gpu_throughput_8gpu"]),
...
best_rows = (
    ...
    .sort_values(["model_label", "per_gpu_throughput_8gpu"], ascending=[True, False])
