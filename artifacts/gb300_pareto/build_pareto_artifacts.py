...
for (model_label, mode), group in raw.groupby(["model_label", "serving_mode"]):
    ax.scatter(
        group["per_user_tps"],
        group["output_token_throughput_72gpu"],
        s=36,
        alpha=0.35,
        color=colors.get(model_label, "#6b7280"),
        marker=markers.get(mode, "s"),
...
for model_label, model_frontier in frontier.groupby("model_label"):
    ...
    ax.plot(
        model_frontier["per_user_tps"],
        model_frontier["output_token_throughput_72gpu"],
        color=color,
        linewidth=2.4,
        marker="D",
        markersize=5.5,
    ...
    for _, row in model_frontier.tail(1).iterrows():
        ...
        ax.annotate(
            label,
            (row["per_user_tps"], row["output_token_throughput_72gpu"]),
            textcoords="offset points",
            xytext=(7, 7),
            fontsize=8.5,
            color=color,
...
for _, row in best_rows.iterrows():
    ax.scatter(
        [row["per_user_tps"]],
        [row["output_token_throughput_72gpu"]],
        s=110,
        facecolors="none",
        edgecolors="#111827",
        linewidths=1.8,
...
ax.set_title("GB300 SOTA Per-Model Pareto Frontiers", fontsize=16, pad=14)
ax.set_xlabel("Per-user TPS (tokens/s/user)")
ax.set_ylabel("Output Token Throughput (tokens/s, 72-GPU normalized)")
ax.grid(True, color="#e5e7eb")
